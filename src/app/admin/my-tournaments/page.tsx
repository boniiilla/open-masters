...
function TModal({ mode, t, onClose, onSave }: { mode: 'create' | 'edit'; t?: Tournament; onClose(): void; onSave(f: FormState): void }) {
  const [form, setForm] = useState<FormState>({
    ...
    modality: t?.modality || 'STATIC',
    knockoutFormat: t?.knockoutFormat || 'SINGLE_KO',
    status: t?.status || 'DRAFT',
    locationName: t?.locationName || '',
    maxTeams: t?.maxTeams?.toString() || '8',
    photo: t?.photo || null,
  })
  ...
  return (
    <div onClick={onClose} style={{
      ...
      backdropFilter: 'blur(6px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #1c1c2c', flexShrink: 0 }}>
          <button onClick={onClose} style={{ fontFamily: 'inherit', fontSize: 14, fontWeight: 500, color: '#8888aa', background: 'none', border: 'none', cursor: 'pointer', padding: '6px 10px', borderRadius: 8 }}>
            Cancelar
          </button>
          ...
          <button onClick={() => onSave(form)} style={{
            ...
            fontSize: 13, fontWeight: 600, border: 'none', borderRadius: 9999,
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 13 }}>
          ...
          <label style={{ cursor: 'pointer' }}
            ...
            <div style={{
              height: 190, border: `2px dashed ${imgHover ? '#5070ff' : '#3b5bff'}`, borderRadius: 14,
              background: imgHover ? 'rgba(59,91,255,.09)' : 'rgba(59,91,255,.04)',
              display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8,
              overflow: 'hidden', transition: 'background 140ms,border-color 140ms',
            }}>
              {form.photo ? (
                ...
                <>
                  ...
                  <p style={{ fontSize: 13.5, fontWeight: 500, color: '#9898ba', margin: 0 }}>
                    {imgHover ? 'Haz clic para subir imagen' : 'Cambiar imagen del torneo'}
                  </p>
          ...
          {/* Name */}
          <div><label style={fl}>NOMBRE DEL TORNEO</label>
            <input style={fi} placeholder="Nombre del torneo" value={form.name} onChange={set('name')} />
          </div>

          {/* Description */}
          <div><label style={fl}>DESCRIPCIÓN</label>
            <textarea style={{ ...fi, resize: 'vertical', minHeight: 76, lineHeight: 1.55 }} placeholder="Descripción del torneo" value={form.description} onChange={set('description')} />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 11 }}>
            <div><label style={fl}>ESTADO</label>
              <select style={{ ...fi, cursor: 'pointer' }} value={form.status} onChange={set('status')}>
                {[['DRAFT','Borrador'],['OPEN','Abierto'],['IN_PROGRESS','En Progreso'],['FINISHED','Finalizado'],['CANCELLED','Parado']].map(([v,l]) => (
                  <option key={v} value={v} style={{ background: '#191926' }}>{l}</option>
                ))}
              </select>
            </div>
            <div><label style={fl}>FORMATO</label>
              <select style={{ ...fi, cursor: 'pointer' }} value={form.knockoutFormat} onChange={set('knockoutFormat')}>
                {[['SINGLE_KO','Simple KO'],['DOUBLE_KO','Doble KO (con repesca)'],['NONE','Sin eliminatoria']].map(([v,l]) => (
                  <option key={v} value={v} style={{ background: '#191926' }}>{l}</option>
                ))}
              </select>
            </div>
          ...
          <div><label style={fl}>MODALIDAD</label>
            <select style={{ ...fi, cursor: 'pointer' }} value={form.modality} onChange={set('modality')}>
              {[['STATIC','Parado'],['MOVEMENT','Movimiento'],['COMBINED','Combinado']].map(([v,l]) => (
                <option key={v} value={v} style={{ background: '#191926' }}>{l}</option>
              ))}
            </select>
          </div>
          ...
          {/* Location */}
          <div><label style={fl}>UBICACIÓN</label>
            <input style={fi} placeholder="Nombre del local o ubicación" value={form.locationName} onChange={set('locationName')} />
          </div>

          {/* Max teams */}
          <div><label style={fl}>MÁXIMO DE EQUIPOS</label>
            <input style={fi} type="number" min={2} max={256} value={form.maxTeams} onChange={set('maxTeams')} />
          </div>
        </div>
      </div>
    </div>
...
function DeleteConfirm({ name, onConfirm, onCancel }: { name: string; onConfirm(): void; onCancel(): void }) {
  return (
    <div onClick={onCancel} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.72)', backdropFilter: 'blur(6px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <style>{`@keyframes slideUp{from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <div onClick={e => e.stopPropagation()} style={{
        background: '#16162a', border: '1px solid #2a2a42', borderRadius: 18,
        width: 360, maxWidth: 'calc(100vw - 32px)', padding: '28px 28px 24px',
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16,
        textAlign: 'center', animation: 'slideUp 200ms cubic-bezier(.16,1,.3,1)',
      }}>
        ...
        <p style={{ fontSize: 13.5, color: '#7070a0', lineHeight: 1.5 }}>
          ¿Seguro que quieres eliminar <strong style={{ color: '#e0e0f0' }}>"{name}"</strong>? Esta acción no se puede deshacer.
        </p>
        <div style={{ display: 'flex', gap: 10, width: '100%' }}>
          <button onClick={onCancel} style={{ flex: 1, padding: 10, borderRadius: 9999, border: '1px solid #2a2a42', background: 'transparent', color: '#9090b0', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 500, cursor: 'pointer' }}>
            Cancelar
          </button>
          <button onClick={onConfirm} style={{ flex: 1, padding: 10, borderRadius: 9999, border: 'none', background: '#ff3a3a', color: '#fff', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>
            Eliminar
          </button>
        </div>
      </div>
...
function Toast({ msg }: { msg: string }) {
  return (
    <div style={{
      ...
      background: '#1b2660', border: '1px solid #304090', color: '#a0b8ff',
...
export default function MyTournamentsPage() {
  ...
  useEffect(() => {
    ...
    if (session.user.role !== 'ADMIN' && session.user.role !== 'SUPERADMIN') { router.push('/'); return }
    fetchTournaments()
  }, [session, status])
  ...
  async function fetchTournaments() {
    ...
    if (data.success) {
      setTournaments(data.data.filter((t: Tournament) => t.creator.id === session?.user?.id))
    }
    setLoading(false)
  }
  ...
  if (status === 'loading' || !session || (session.user.role !== 'ADMIN' && session.user.role !== 'SUPERADMIN') || loading) {
    return (
  ...
  return (
    <div className="animate-fade-in px-5 md:px-0" style={{ paddingBottom: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, paddingTop: 8 }}>
        <div>
          <h1 style={{ fontSize: 38, fontWeight: 800, letterSpacing: '-.025em', color: '#fff', lineHeight: 1 }}>Mis Torneos</h1>
          <p style={{ fontSize: 13, color: '#666688', marginTop: 5 }}>Gestiona tus torneos creados</p>
        </div>
      ...
      <div style={{ position: 'relative', marginBottom: 18 }}>
        ...
        <input
          type="text" placeholder="Buscar torneos..." value={q} onChange={e => setQ(e.target.value)}
          style={{
      ...
      {visible.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 20px', gap: 12, color: '#444460', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 15 }}>
            {q || tab !== 'all' ? 'No hay torneos que coincidan' : 'No has creado ningún torneo todavía'}
          </p>
          {tab === 'all' && !q && (
